...
class NimOSAPIClient:
    ...
    _ENDPOINTS = {
        ...
        'client_credentials': 'v1/client_credentials',
    ...
    def __init__(self, hostname, username, password, app_name=None, job_timeout=60, port=5392, tenant_aware=False):
        ...
        if tenant_aware and port == 5392:
            port = 443
        connection_hash = str(uuid.uuid3(
            uuid.NAMESPACE_OID, f'{hostname}{port}{username}{password}'))
        ...
        logging.debug(
            f"NimOSAPIClient created with [hostname: {hostname}], [job_timeout: {job_timeout} seconds], [port: {port}], [SDK Version: v{__version__}]")
    ...
    def _connect(self):
        ...
        endpoint = f"api/tenant/{self._ENDPOINTS['tokens']}" if self.tenant_aware else self._ENDPOINTS['tokens']
        try:
            ...
            SessionManager._SESSIONS[self.__connection_hash] = (
    ...
    def _refresh_connection(self):
        ...
        try:
            response = requests.get(
                f"https://{self.hostname}:{self.port}/{endpoint}/{self.session_id}",
    ...
    def get(self, endpoint, filter=None, limit=None, from_id=None, **params):
        ...
        endpoint = f"api/tenant/{endpoint}" if self.tenant_aware else endpoint
        ...
        try:
            ...
            while 1:
                ...
                logging.debug(
                    f"[read_records_count: {len(response_data)}], [total_rows: {response['totalRows']}], [next_start_row: {params['startRow']}]")
            ...
            raise ConnectionError(
                f"Error retrieving data from {self.hostname}")

    def delete(self, endpoint, job_timeout=None):
        """Wrapper for DELETE requests"""

        logging.debug(
            f"job_timeout: [{job_timeout if job_timeout is not None else self.job_timeout} seconds]")
        endpoint = f"api/tenant/{endpoint}" if self.tenant_aware else endpoint
        try:
            while 1:
                ...
                if response.status_code >= 400:
                    ...
                    if 'messages' in job_response and job_response['messages'][0]['code'] == 'SM_async_job_id':
                        logging.debug("Got async job response: %s", json.dumps(
                            job_response['messages'], indent=4))
    ...
    def put(self, endpoint, job_timeout=None, **payload):
        ...
        if 'metadata' in payload:
            payload['metadata'] = [
                {'key': key, 'value': payload['metadata'][key]} for key in payload['metadata']]
    ...
    def post(self, endpoint, job_timeout=None, **params):
        ...
        logging.debug(
            "job_timeout: [%s seconds]", job_timeout if job_timeout is not None else self.job_timeout)

        # Translate metadata dict to REST key-value pairs.
        # Ex: {'k1': 'k1', 'k2': 'v2'} ==> [{'key': 'k1', 'value': 'v1'}, {'key': 'k2', 'value': 'v2'}]
        if 'metadata' in params:
            params['metadata'] = [
                {'key': key, 'value': params['metadata'][key]} for key in params['metadata']]
    ...
    def list_resources(self, resource, detail=False, filter=None, limit=None, from_id=None, **params):
        ...
        if filter is not None:
            filter = self.build_advanced_criteria(
                filter['operator'], filter['criteria'])

        resp = self.get(
            f"{self._ENDPOINTS[resource]}{'/detail' if detail else ''}", filter, limit, from_id, **params)
        return resp['data'] if 'data' in resp else resp
    ...
    def perform_resource_action(self, resource, ident, action, **params):
        ...
        resp = self.post(
            f"{self._ENDPOINTS[resource]}/{ident}/actions/{action}", **params)
        return resp['data'] if 'data' in resp else resp
    ...
    def perform_bulk_resource_action(self, resource, action, **params):
        ...
        resp = self.post(
            f"{self._ENDPOINTS[resource]}/actions/{action}", **params)
        return resp['data'] if 'data' in resp else resp
    ...
    def job_handler(self, job_id, job_timeout, retry_interval=5):
        ...
        while 1:
            try:
                ...
                logging.debug("JOB STATUS RESPONSE: %s",
                              json.dumps(response, indent=4))
                if (response['state'] == 'done'):
                    logging.info(
                        "Job with id '%s' completed with status '%s'", job_id, response['result'])
                    break

                if datetime.datetime.now().time() > expiry_time.time():
                    logging.error(
                        "Job status retry timeout has expired after [%s] seconds, returning with client timeout exception", timeout_sec)
                    raise NimOSClientJobTimeoutError(response)
                else:
                    #  Sleep n Wait
                    if timeout_sec < retry_interval:
                        logging.debug(
                            "Sleeping for [%s seconds] before next retry attempt ...", timeout_sec)
                        time.sleep(timeout_sec)
                    else:
                        logging.debug(
                            "Sleeping for [%s seconds] before next retry attempt ...", retry_interval)
                        time.sleep(retry_interval)

            except requests.exceptions.RequestException as error:
                logging.exception(error)
                raise ConnectionError(
                    f"Error communicating with {self.hostname}")
