...
class Volume(Resource):
    ...
    - warn_level                    : This attribute is deprecated. Alert threshold for the volume's mapped usage, expressed as a percentage of the volume's size. When the
                                      volume's mapped usage exceeds warn_level, the array issues an alert. If this option is not specified, array default volume warn level setting
                                      is used to decide the warning level for this volume.
    - limit                         : Limit on the volume's mapped usage, expressed as a percentage of the volume's size. When the volume's mapped usage exceeds limit, the volume
                                      will be offlined or made non-writable. If this option is not specified, array default volume limit setting is used to decide the limit for
                                      this volume.
    ...
    - snap_limit                    : This attribute is deprecated. The array does not limit a volume's snapshot space usage. The attribute is ignored on input and returns max
                                      int64 on output.
    ...
    - iscsi_target_scope            : This indicates whether volume is exported under iSCSI Group Target or iSCSI Volume Target. This attribute is only meaningful to iSCSI system.
                                      On FC system, all volumes are exported under the FC Group Target. In create operation, the volume's target type will be set by this
                                      attribute. If not specified, it will be set as the group-setting. In clone operation, the clone's target type will inherit from the parent'
                                      setting.
    ...
    - thinly_provisioned            : Set volume's provisioning level to thin.  Also advertises volume as thinly provisioned to initiators supporting thin provisioning. For such
                                      ...
                                      space is provisioned immediately, but for advertising status, this change takes effect only for new connections to the volume. Initiators
                                      must disconnect and reconnect for the new setting to be take effect at the initiator level consistently.
    ...
    - limit_iops                    : IOPS limit for this volume. If limit_iops is not specified when a volume is created, or if limit_iops is set to -1, then the volume has no
                                      IOPS limit. If limit_iops is not specified while creating a clone, IOPS limit of parent volume will be used as limit. IOPS limit should be in
                                      range [256, 4294967294] or -1 for unlimited. If both limit_iops and limit_mbps are specified, limit_mbps must not be hit before limit_iops.
                                      In other words, IOPS and MBPS limits should honor limit_iops <= ((limit_mbps MB/s * 2^20 B/MB) / block_size B).
    ...
    def move(self, dest_pool_id, force_vvol=False):
        ...
        - force_vvol   : Forcibly move a Virtual Volume. Moving Virtual Volume is disruptive to the vCenter, hence it should only be done by the VASA Provider (VP). This flag
                         should only be set by the VP when it calls this API.
        '''
...
class VolumeList(Collection):
    ...
    def restore(self,
        ...
        return self._client.perform_resource_action(self.resource_type, id, 'restore',
    ...
    def move(self,
        ...
        return self._client.perform_resource_action(self.resource_type, id, 'move',
    ...
    def bulk_move(self,
        ...
        return self._client.perform_bulk_resource_action(self.resource_type, 'bulk_move',
                                                         dest_pool_id=dest_pool_id,
                                                         vol_ids=vol_ids,
                                                         force_vvol=force_vvol
                                                         )
    ...
    def abort_move(self,
        ...
        return self._client.perform_resource_action(self.resource_type, id, 'abort_move',
                                                    id=id
                                                    )

    def bulk_set_dedupe(self,
                        dedupe_enabled,
                        vol_ids
                        ):
        '''
        ...
        return self._client.perform_bulk_resource_action(self.resource_type, 'bulk_set_dedupe',
                                                         dedupe_enabled=dedupe_enabled,
                                                         vol_ids=vol_ids
                                                         )

    def bulk_set_online_and_offline(self,
                                    online,
                                    vol_ids
                                    ):
        '''
        ...
        return self._client.perform_bulk_resource_action(self.resource_type, 'bulk_set_online_and_offline',
                                                         online=online,
                                                         vol_ids=vol_ids
                                                         )
    ...
    def associate(self, id, volcoll):
        ...
        return self._client.update_resource(self.resource_type, id,
